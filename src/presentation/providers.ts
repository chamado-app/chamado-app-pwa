export enum PROVIDERS {
  // Usecases
  AUTHENTICATE_USECASE = 'authenticateUsecase',
  CANCEL_TICKET_USECASE = 'cancelTicketUsecase',
  CHANGE_TICKET_ASSIGNED_USECASE = 'changeTicketAssignedUsecase',
  CHANGE_TICKET_STATUS_USECASE = 'changeTicketStatusUsecase',
  COMPLETE_TICKET_USECASE = 'completeTicketUsecase',
  CREATE_CATEGORY_USECASE = 'createCategoryUsecase',
  CREATE_EQUIPMENT_USECASE = 'createEquipmentUsecase',
  CREATE_TICKET_USECASE = 'createTicketUsecase',
  CREATE_USER_USECASE = 'createUserUsecase',
  DELETE_CATEGORY_USECASE = 'deleteCategoryUsecase',
  DELETE_EQUIPMENT_USECASE = 'deleteEquipmentUsecase',
  DELETE_USER_USECASE = 'deleteUserUsecase',
  FETCH_CATEGORIES_USECASE = 'fetchCategoriesUsecase',
  FETCH_EQUIPMENTS_USECASE = 'fetchEquipmentsUsecase',
  FETCH_USERS_USECASE = 'fetchUsersUsecase',
  LIST_CATEGORIES_USECASE = 'listCategoriesUsecase',
  LIST_EQUIPMENTS_USECASE = 'listEquipmentsUsecase',
  LIST_TICKETS_USECASE = 'listTicketsUsecase',
  LIST_USERS_USECASE = 'listUsersUsecase',
  LOGOUT_USECASE = 'logoutUsecase',
  SEND_TICKET_TEXT_MESSAGE_USECASE = 'sendTicketTextMessageUsecase',
  SHOW_CATEGORY_USECASE = 'showCategoryUsecase',
  SHOW_EQUIPMENT_USECASE = 'showEquipmentUsecase',
  SHOW_TICKET_USECASE = 'showTicketUsecase',
  SHOW_USER_USECASE = 'showUserUsecase',
  UPDATE_CATEGORY_USECASE = 'updateCategoryUsecase',
  UPDATE_EQUIPMENT_USECASE = 'updateEquipmentUsecase',
  UPDATE_USER_USECASE = 'updateUserUsecase',

  // Utils
  NOTIFIER = 'notifier'
}
